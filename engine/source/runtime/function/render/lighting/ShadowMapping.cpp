...
ShadowMapping::ShadowMapping(const string& shaderPath)
: RenderPass(shaderPath){
    shaderDepth = std::make_shared<Shader>(PU::getFullPath(g_global_context.m_config_manager->getShaderFolder(), "common/depth.vert").c_str()
            ,PU::getFullPath(g_global_context.m_config_manager->getShaderFolder(), "common/depth.frag").c_str());
...
void ShadowMapping::initialize(){
    ...
    lightCamera = make_shared<Camera>(CameraType::ORTHOGONAL);
    lightCamera->nearPlane = 1.0f;
    lightCamera->farPlane = 7.5f;
    lightCamera->setCameraPosition(glm::vec3(-2.0f, 4.0f, -1.0f));
...
void ShadowMapping::drawScene(Shader& curShader){
    // floor
    glm::mat4 model = glm::mat4(1.0f);
    curShader.setValue("model", model);
    glBindVertexArray(planeVAO);
...
void ShadowMapping::draw(Camera& camera){
    glm::mat4 lightSpaceMatrix = lightCamera->getProjectionMatrix()*lightCamera->getCameraPoseMatrix();

    // 1. render to depth map
    shaderDepth->use();
    shaderDepth->setValue("lightSpaceMatrix",lightSpaceMatrix);
